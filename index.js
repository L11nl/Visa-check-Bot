...
// وكلاء الاتصال لحل مشكلة البطء عند تعدد المستخدمين ومنع التهنيج
const httpAgent = new http.Agent({ keepAlive: true, maxSockets: 1000 });
const httpsAgent = new https.Agent({ keepAlive: true, maxSockets: 1000, rejectUnauthorized: false });
...
async function checkSingleCard(cardString, chatId, isSilent=false, retries=2) { 
    try { 
        ...
        if(!isSilent){ 
            ...
            await bot.sendMessage(chatId,translations[l].api_error); 
        } 
        return { isLive: false }; 
    } 
}
...
async function runMultipleCampaignsParallel(chatId, binsList, totalCardsPerBin) {
    ...
    const runSingle = async (camp) => {
        ...
        const portResults = await Promise.all(portVisas.map(async (visas, idx) => {
            let portHit = 0;
            const batchSize = 5; 
            const delayMs = speedPercent <= 10 ? 1500 : (speedPercent <= 50 ? 500 : 100);
